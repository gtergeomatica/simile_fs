...
Glb= {'debug': True, 'my_app_name': "simile_fs", 'tte_path': '/static/tte'}


def before_request():
    bad_path= '/simile_fs/static/tte/simile_fs/static/tte/'
    bad_root='/simile_fs/simile_fs/'
    if bottle.request.environ["PATH_INFO"].startswith(bad_path):
        bottle.request.environ["PATH_INFO"]= bottle.request.environ["PATH_INFO"].replace( '/simile_fs/static/tte', '', 1)
        print ( f'before_request:    /simile_fs: goto {bottle.request.environ["PATH_INFO"]}' )
    elif bottle.request.environ["PATH_INFO"].startswith(bad_root):
        bottle.request.environ["PATH_INFO"]= bottle.request.environ["PATH_INFO"].replace( '/simile_fs', '', 1)
        print ( f'before_request:    /simile_fs: goto {bottle.request.environ["PATH_INFO"]}' )
...
#
# curl -X  GET   http://localhost:8000/simile_fs/api/test_table/
# curl -X  GET   http://localhost:8000/simile_fs/api/test_table/9
# curl -X DELETE  http://localhost:8000/simile_fs/api/test_table/2
# curl -X POST -d 'f0=1111111&f1=2222222222&f2=33333333333' http://localhost:8000/simile_fs/api/test_table/
# curl -X PUT -d 'f0=1111111&f1=2222222222&f2=33333333333' http://localhost:8000/simile_fs/api/test_table/19
# curl -X POST -d f0=1111111   -d f1=2222222222 -d f2=8888888888  http://localhost:8000/simile_fs/api/test_table/
#
#  pip install httpie
#  http         localhost:8000/simile_fs/api/test_table/
